...
function emptyForm() {
  return { user_id: 0, username: '', role_id: defaultReaderRoleID(), organization_id: Number(props.selectedOrganizationID) || 0, mfa_enrollment_required: false }
}
function defaultReaderRoleID() {
  return Number(props.roles.find((role) => role.code === 'reader')?.ID) || 0
}
...
function submit() {
  const payload = {
    ...
    role_id: Number(form.value.role_id) || 0,
      ...
      <section class="modal-card" role="dialog" aria-modal="true" :aria-labelledby="`${dialogMode}-member-dialog-title`">
        ...
        <form class="form-stack" @submit.prevent="submit">
          <template v-if="dialogMode === 'create'">
            ...
            <p class="form-hint">添加成员时会同时授予所选角色。若存在代码为 <code>reader</code> 的只读角色，会自动预选；请确认其菜单与 API 权限已配置完成。</p>
          </template>
          <template v-else>
