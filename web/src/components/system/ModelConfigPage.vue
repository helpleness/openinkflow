...
const settings = reactive({
  provider_type: 'openai-compatible', base_url: '', api_key: '', has_api_key: false, model_default: '',
  ocr_provider_type: '', ocr_base_url: '', ocr_api_key: '', has_ocr_api_key: false, ocr_model: '',
})

function assign(data = {}) {
  Object.assign(settings, {
    provider_type: data.provider_type || 'openai-compatible', base_url: data.base_url || '', api_key: '', has_api_key: Boolean(data.has_api_key), model_default: data.model_default || '',
    ocr_provider_type: data.ocr_provider_type || '', ocr_base_url: data.ocr_base_url || '', ocr_api_key: '', has_ocr_api_key: Boolean(data.has_ocr_api_key), ocr_model: data.ocr_model || '',
  })
}
  ...
  <section class="model-config-page">
    ...
    <form class="model-grid" @submit.prevent="save">
      <section class="model-card primary-model">
        ...
        <p>用于写作、检索总结等主要工作流。供应商类型决定能力和协议扩展，不会从 URL 猜测。</p>
        <label>供应商类型<select v-model="settings.provider_type"><option value="openai">OpenAI</option><option value="newapi">NewAPI</option><option value="deepseek">DeepSeek</option><option value="qwen">Qwen</option><option value="openai-compatible">OpenAI Compatible</option></select></label>
      ...
      <section class="model-card ocr-model">
        ...
        <label>语义模型供应商类型<select v-model="settings.ocr_provider_type"><option value="">复用主模型</option><option value="openai">OpenAI</option><option value="newapi">NewAPI</option><option value="deepseek">DeepSeek</option><option value="qwen">Qwen</option><option value="openai-compatible">OpenAI Compatible</option></select></label>
...
.model-card input, .model-card select { width: 100%; min-height: 44px; padding: 0 12px; border: 1px solid #cfddd4; border-radius: 9px; outline: 0; color: #192820; background: #fbfdfb; font: inherit; transition: border-color .18s ease, box-shadow .18s ease, background .18s ease; }
.model-card input:focus, .model-card select:focus { border-color: #619a80; background: #fff; box-shadow: 0 0 0 4px rgba(56, 130, 96, .1); }
