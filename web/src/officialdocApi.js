...
export function updateWritingTaskManualVersion(tenantID, taskID, versionID, payload) {
  return apiJson(`${API_PREFIX}/officialdoc/writing-tasks/${taskID}/versions/${versionID}`, payload, 'PUT', tenantOptions(tenantID))
}
...
export function getDocumentVersionDiff(tenantID, taskID, versionID, baseVersionID = 0) {
  const query = Number(baseVersionID) > 0 ? { base_version_id: Number(baseVersionID) } : {}
  return apiGet(`${API_PREFIX}/officialdoc/writing-tasks/${taskID}/versions/${versionID}/diff`, query, tenantOptions(tenantID))
}
