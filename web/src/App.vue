...
import WorkspaceApplicationPage from './components/system/WorkspaceApplicationPage.vue'
import WorkspaceApplicationReviewPage from './components/system/WorkspaceApplicationReviewPage.vue'
import WorkspaceSettingsPage from './components/system/WorkspaceSettingsPage.vue'
import CreateWorkspacePage from './components/system/CreateWorkspacePage.vue'
...
import {
  ...
  applyToWorkspace,
  fetchWorkspaceApplicationReviews,
  reviewWorkspaceApplication as reviewWorkspaceApplicationRequest,
  fetchTenantSettings,
...
const user = ref(null)
const tenants = ref([])
const publicWorkspaces = ref([])
const workspaceApplications = ref([])
const workspaceApplicationReviews = ref([])
...
const FRONTEND_MENU_DEFINITIONS = [
  ...
  { name: '工作空间审核', menu_key: 'workspace_application_reviews', parent_key: 'permission_management', view_key: 'workspace_application_reviews', description: '审核新账号加入当前工作空间的申请', sort: 125, is_enabled: true },
  { name: '工作空间设置', menu_key: 'workspace_settings', parent_key: 'permission_management', view_key: 'workspace_settings', description: '设置工作空间是否允许申请加入和审批默认角色', sort: 128, is_enabled: true },
...
const canManage = computed(() => menus.value.includes('roles') || menus.value.includes('workspace_settings'))
// A child can only be reached when it and every one of its ancestors is
// enabled. Besides keeping the sidebar coherent, this protects navigation
// while an administrator hides the very page currently being viewed.
const configuredMenuItems = computed(() => {
  const allMenus = systemMenus.value.length ? systemMenus.value : FRONTEND_MENU_DEFINITIONS
  const byKey = new Map(allMenus.map((item) => [item.menu_key, item]))
  return allMenus.filter((item) => {
    if (item.is_enabled === false) return false
    const visited = new Set([item.menu_key])
    let parentKey = item.parent_key
    while (parentKey) {
      if (visited.has(parentKey)) return false
      visited.add(parentKey)
      const parent = byKey.get(parentKey)
      if (!parent || parent.is_enabled === false) return false
...
async function selectTenant(event) {
  const tenantID = Number(event.target.value) || 0
  if (!tenantID || tenantID === selectedTenantID.value) return
  selectedTenantID.value = tenantID
  selectedOrganizationID.value = 0
  assignedOrganizationID.value = 0
  await loadTenants(false, tenantID)
}
function menuIcon(menuKey) { return navigationIconMap[menuKey] || Sparkles }
function isActiveViewVisible() {
  if (activeView.value === 'workspace') return true
  return configuredMenuItems.value.some((item) => item.view_key === activeView.value && menus.value.includes(item.menu_key))
}
function returnToWorkspaceWhenCurrentViewIsHidden() {
  if (isActiveViewVisible()) return false
  activeView.value = 'workspace'
  return true
}
function selectView(view, parentKey = '') {
  activeView.value = view
  if (view === 'workspace_applications') void loadWorkspaceApplications()
  ...
  try {
    ...
    const result = await authenticate(authMode.value, {
      ...
      captcha_answer: credentials.value.captcha_answer,
		...
		await finishAuthentication(result, registering ? '账号已创建。请选择公开工作空间并提交加入申请。' : '登录成功。')
  } catch (requestError) {
...
async function loadTenants(silent = false, preferredTenantID = 0) {
  try {
    const [currentUser, items] = await Promise.all([fetchCurrentUser(), fetchTenants()])
    user.value = currentUser
    tenants.value = items || []
    if (preferredTenantID && tenants.value.some((item) => item.ID === preferredTenantID)) selectedTenantID.value = preferredTenantID
    else if (!tenants.value.some((item) => item.ID === selectedTenantID.value)) selectedTenantID.value = tenants.value[0]?.ID || 0
    if (!selectedTenantID.value) {
      menus.value = []
      systemMenus.value = []
      assignedOrganizationID.value = 0
      selectedOrganizationID.value = 0
      if (!['workspace_applications', 'create_workspace'].includes(activeView.value)) activeView.value = 'workspace_applications'
      await loadWorkspaceApplications()
      return
    }
    if (activeView.value === 'workspace_applications') activeView.value = 'workspace'
    if (selectedTenantID.value) {
...
async function loadWorkspaceApplications() {
  systemLoading.value = true
  try {
    const [workspaces, applications] = await Promise.all([fetchPublicWorkspaces(), fetchWorkspaceApplications()])
    publicWorkspaces.value = Array.isArray(workspaces) ? workspaces : []
    workspaceApplications.value = Array.isArray(applications) ? applications : []
  } catch (requestError) { showError(requestError.message) } finally { systemLoading.value = false }
}

async function loadTenantResources() {
  if (!selectedTenantID.value) {
    organizations.value = []; publicOrganizations.value = []; roles.value = []; memberships.value = []; globalUsers.value = []; membershipApplications.value = []; workspaceApplicationReviews.value = []; tenantSettings.value = null; selectedOrganizationID.value = 0; auditLogs.value = []; apiResources.value = []; systemMenus.value = []
    return
  }
  systemLoading.value = true
  try {
    ...
    if (menus.value.includes('roles') || menus.value.includes('members') || menus.value.includes('workspace_settings')) {
    ...
    if (menus.value.includes('workspace_application_reviews')) requests.push(fetchWorkspaceApplicationReviews(selectedTenantID.value).then((items) => { workspaceApplicationReviews.value = Array.isArray(items) ? items : [] }).catch(() => { workspaceApplicationReviews.value = [] }))
    else workspaceApplicationReviews.value = []
    if (menus.value.includes('workspace_settings')) requests.push(fetchTenantSettings(selectedTenantID.value).then((item) => { tenantSettings.value = item || null }).catch(() => { tenantSettings.value = null }))
    else tenantSettings.value = null
...
async function applyToWorkspaceFromPage(tenantID) {
  clearMessage()
  try { await applyToWorkspace(tenantID); notice.value = '工作空间申请已提交，等待管理员审核。'; await loadWorkspaceApplications() } catch (requestError) { showError(requestError.message) }
}

async function reviewWorkspaceApplication(payload) {
  clearMessage()
  try { await reviewWorkspaceApplicationRequest(selectedTenantID.value, payload.id, { approve: payload.approve }); notice.value = payload.approve ? '申请已批准，成员已获得默认角色。' : '申请已拒绝。'; await loadTenantResources() } catch (requestError) { showError(requestError.message) }
}

async function saveTenantSettings(payload) {
  clearMessage()
  try { await updateTenantSettings(selectedTenantID.value, payload); notice.value = payload.is_visible ? '工作空间已开放申请，默认角色已保存。' : '工作空间已停止接受新申请。'; await loadTenantResources() } catch (requestError) { showError(requestError.message) }
}
...
async function createWorkspace(payload) {
  ...
  systemLoading.value = true
  try {
    const result = await createTenant({ name: payload.name, code: payload.code })
    selectedTenantID.value = Number(result?.tenant?.ID || 0)
    activeView.value = 'workspace'
    notice.value = '工作空间已创建；你已成为所有者。'
    await loadTenants(false, selectedTenantID.value)
  } catch (requestError) { showError(requestError.message) } finally { systemLoading.value = false }
}
...
async function updateMenuFromPage(payload) {
  clearMessage()
  try {
    const { id, ...menu } = payload
    await updateMenuConfig(selectedTenantID.value, id, menu)
    systemMenus.value = await fetchMenuConfigs(selectedTenantID.value) || []
    notice.value = returnToWorkspaceWhenCurrentViewIsHidden()
      ? '菜单已隐藏，已安全返回工作台。'
      : '前端菜单已更新。'
  } catch (requestError) { showError(requestError.message) }
}
...
async function signOut() {
	await logout()
	user.value = null; tenants.value = []; publicWorkspaces.value = []; workspaceApplications.value = []; workspaceApplicationReviews.value = []; tenantSettings.value = null; organizations.value = []; publicOrganizations.value = []; roles.value = []; memberships.value = []; auditLogs.value = []; apiResources.value = []; systemMenus.value = []; menus.value = []; selectedTenantID.value = 0; selectedOrganizationID.value = 0; assignedOrganizationID.value = 0; activeView.value = 'workspace'; notice.value = '已退出当前会话。'
}
...
<template>
  ...
  <main v-if="!user" class="auth-shell">
    ...
    <section class="auth-card" aria-labelledby="auth-title">
      ...
      <p class="auth-note">注册仅创建账号；请在登录后申请加入公开工作空间，审核通过后由管理员预设角色授权。登录受图片验证码、失败锁定和安全 Cookie 保护。</p>
    </section>
  </main>
  ...
  <main v-else class="app-shell">
    <aside class="sidebar">
      ...
      <nav class="side-nav">
        ...
        <button :class="['nav-item', { active: activeView === 'workspace_applications' }]" type="button" @click="selectView('workspace_applications')"><span class="nav-icon"><Building2 :size="17" /></span><span>申请加入工作空间</span><ChevronRight class="nav-item-arrow" :size="15" /></button>
        <button :class="['nav-item', { active: activeView === 'create_workspace' }]" type="button" @click="selectView('create_workspace')"><span class="nav-icon"><Building2 :size="17" /></span><span>新建工作空间</span><ChevronRight class="nav-item-arrow" :size="15" /></button>
      </nav>
    ...
    <section class="content">
      <header class="topbar">
        ...
        <div class="topbar-actions"><label class="tenant-select"><span>当前工作空间</span><div class="select-shell"><select :value="selectedTenantID" @change="selectTenant" :disabled="!tenants.length || systemLoading">
            ...
            <option v-for="tenant in tenants" :key="tenant.ID" :value="tenant.ID">{{ tenant.name }} · {{ tenant.code }}</option>
          </select><ChevronDown :size="15" /></div></label><label class="tenant-select"><span>当前组织</span><div class="select-shell"><select :value="effectiveOrganizationID" @change="selectOrganization" :disabled="!selectedTenantID || !canManage || (!organizations.length && !effectiveOrganizationID) || systemLoading">
      ...
      <WorkspaceApplicationPage v-if="activeView === 'workspace_applications'" :workspaces="publicWorkspaces" :applications="workspaceApplications" :loading="systemLoading" @apply="applyToWorkspaceFromPage" @refresh="loadWorkspaceApplications" />
      <CreateWorkspacePage v-else-if="activeView === 'create_workspace'" :saving="systemLoading" @create="createWorkspace" />
      <WorkspacePage v-else-if="activeView === 'workspace'" :organization="selectedOrganization" :organizations="publicOrganizations" :applications="membershipApplications" :organization-count="publicOrganizations.length" :member-count="visibleMemberships.length" :role-count="roles.length" :can-manage="canManage" @navigate="selectView($event)" @apply="applyFromWorkspace" />
      ...
      <WorkspaceApplicationReviewPage v-else-if="activeView === 'workspace_application_reviews'" :applications="workspaceApplicationReviews" :loading="systemLoading" @review="reviewWorkspaceApplication" @refresh="loadTenantResources" />
