...
// Keep each WebGPU dispatch small enough for ordinary client GPUs. Every
// candidate is still scored and the results are sorted globally below.
const RERANK_BATCH_SIZE = 4
// The server bounds one hybrid-search candidate union to 24 documents.
const RERANK_MAX_CANDIDATES = 24
const EMBED_RUN_TIMEOUT_MS = 45000
const RERANK_RUN_TIMEOUT_MS = 60000
...
async function rerankDocuments(query, docs, topN = 0) {
  ...
  if (docs.length > RERANK_MAX_CANDIDATES) {
    throw new Error(`rerank candidate count ${docs.length} exceeds limit ${RERANK_MAX_CANDIDATES}`)
  }
  ...
  try {
    ...
    self.postMessage({ type: 'status', message: `Rerank failed: ${error.message}` })
    throw error
  } finally {
