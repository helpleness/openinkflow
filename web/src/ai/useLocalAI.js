...
import { AUTH_SESSION_REPLACED_EVENT, getAPIBase, getAuthToken, revokeAuthTokenForTab, usesCookieSession } from '../api'
...
function websocketURL() {
  ...
  // 浏览器会自动在同源握手中携带 HttpOnly Cookie；仅桌面 loopback 使用查询令牌。
  if (!usesCookieSession()) endpoint.searchParams.set('token', getAuthToken())
...
function connectInferenceSocket() {
  if (!localAI.ready || (!usesCookieSession() && !getAuthToken())) return
