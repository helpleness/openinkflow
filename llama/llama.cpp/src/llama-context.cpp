...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    // Rank pooling only consumes one classifier output per sequence. Respect
    // the caller's batch.logits flags in that mode instead of forcing every
    // token to be an output (which also triggers the "embeddings required ...
    // overriding" warning and needlessly grows the output buffers).
    const bool output_all = cparams.embeddings && cparams.pooling_type != LLAMA_POOLING_TYPE_RANK;
    ...
    const uint32_t n_outputs_batch = balloc->get_n_outputs();
    ...
    if (output_reserve(n_outputs_batch) < n_outputs_batch) {
        LLAMA_LOG_ERROR("%s: could not reserve space for batch with %u outputs\n", __func__, n_outputs_batch);
        return -2;
    };
    ...
    if (output_all) {
        ...
        std::fill(output_ids.begin(), output_ids.begin() + n_tokens, -1);
        const auto & out_ids = balloc->get_out_ids();
        for (uint32_t i = 0; i < out_ids.size(); ++i) {
            output_ids[out_ids[i]] = i;
        }
    }

    n_outputs = n_outputs_batch;
