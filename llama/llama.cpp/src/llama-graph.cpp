...
void llm_graph_input_cls::set_input(const llama_ubatch * ubatch) {
    ...
    if (cparams.embeddings && (
        ...
        for (int s = 0; s < n_seqs_unq; ++s) {
            if (target_row[s] >= 0) {
                // The final transformer layer may have been reduced to the
                // rows selected by batch.logits (rank pooling). Convert the
                // original token row to its compact output-row index before
                // feeding ggml_get_rows().
                if (ubatch->output) {
                    int32_t output_row = 0;
                    for (int i = 0; i < target_row[s]; ++i) {
                        output_row += ubatch->output[i] != 0;
                    }
                    data[s] = output_row;
                } else {
                    data[s] = target_row[s];
