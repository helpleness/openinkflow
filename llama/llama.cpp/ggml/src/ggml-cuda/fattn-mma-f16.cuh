...
template<bool BERT_TUNED>
static constexpr __device__ int ggml_cuda_fattn_mma_get_occupancy_tuned(const int DKQ, const int DV, const int ncols) {
    if constexpr (BERT_TUNED) {
        GGML_UNUSED_VARS(DKQ, DV, ncols);
        // Ask nvcc to target six resident blocks per SM for this fixed
        // reranker shape.  The normal configuration remains unchanged.
        return 6;
    } else {
        return ggml_cuda_fattn_mma_get_occupancy(DKQ, DV, ncols);
    }
}
...
// The BGE reranker uses a fixed BERT attention shape (head dim 64, 16 heads,
// non-GQA). Keep this as a separate template variant so the BERT launch-bound
// tuning can evolve independently from other FlashAttention shapes. The A/B
// test for this shape retains nbatch_fa=32; lowering it reduced throughput.
template<bool BERT_TUNED>
static __host__ int ggml_cuda_fattn_mma_get_nbatch_fa_tuned(const int DKQ, const int DV, const int ncols, const int cc) {
    if constexpr (BERT_TUNED) {
        GGML_ASSERT(DKQ == 64 && DV == 64 && ncols == 64);
        GGML_UNUSED(cc);
        return 32;
    } else {
        return ggml_cuda_fattn_mma_get_nbatch_fa(DKQ, DV, ncols, cc);
    }
}

template<bool BERT_TUNED>
static constexpr __device__ int ggml_cuda_fattn_mma_get_nbatch_fa_tuned(const int DKQ, const int DV, const int ncols) {
    if constexpr (BERT_TUNED) {
        return 32;
    } else {
...
template<int DKQ, int DV, int ncols1, int ncols2, bool BERT_TUNED, int nwarps,
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
    ...
    constexpr int  nbatch_fa       = ggml_cuda_fattn_mma_get_nbatch_fa_tuned<BERT_TUNED>(DKQ, DV, ncols);
    ...
    // Encoder batches contain independent sequences in one dense tensor. Their
    // attention mask is therefore block diagonal. KV_max only removes masked
    // tiles at the right edge; without this check, tiles belonging entirely to
    // earlier sequences still execute both MMA phases before the -inf mask is
    // applied. Reuse the mask already staged in shared memory and let the whole
    // block skip such tiles. This is restricted to the BERT-tuned ncols2 == 1
    // path, whose single-stage loader has no outstanding asynchronous copies.
    if constexpr (BERT_TUNED && ncols2 == 1 && nstages <= 1) {
        if (mask_h) {
            __syncthreads();
            ...
            for (int idx = tid; idx < ncols1 * nbatch_fa; idx += nthreads) {
                const int i = idx % nbatch_fa;
                if (i < k_VKQ_sup) {
                    const int j = idx / nbatch_fa;
                    all_masked &= int(isinf(__half2float(tile_mask[j * (nbatch_fa + 8) + i])));
                }
            }

            if (__syncthreads_and(all_masked)) {
                return;
            }
        }
    }
...
template<int DKQ, int DV, int ncols1, int ncols2, bool BERT_TUNED, int nwarps, bool use_logit_softcap, bool V_is_K_view, bool needs_fixup, bool is_fixup>
        ...
        if constexpr (ncols2 == 1) {
            ...
            for (; kb0 < kb0_stop-1; ++kb0) {
                ...
                flash_attn_ext_f16_iter
                    <DKQ, DV, ncols1, ncols2, BERT_TUNED, nwarps, use_logit_softcap, V_is_K_view, needs_fixup, is_fixup, last_iter, oob_check,
...
template<int DKQ, int DV, int ncols1, int ncols2, bool BERT_TUNED, bool use_logit_softcap, bool V_is_K_view>
__launch_bounds__(ggml_cuda_fattn_mma_get_nthreads(DKQ, DV, ncols1*ncols2), ggml_cuda_fattn_mma_get_occupancy_tuned<BERT_TUNED>(DKQ, DV, ncols1*ncols2))
static __global__ void flash_attn_ext_f16(
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        const int kb0_stream_start = kb0_start;
        if (KV_max) {
            const int bounds_index = sequence*iter_j + jt;
            kb0_stop = min(kb0_stop, KV_max[bounds_index] / nbatch_fa);
            if constexpr (BERT_TUNED) {
                const int bounds_count = iter_j*ne03;
                kb0_start = max(kb0_start, KV_max[bounds_count + bounds_index] / nbatch_fa);
            }
        }
        constexpr bool is_fixup = false; // All but (potentially) the last iterations write their data to dst rather than the fixup buffer.
        if (kb0_stream_start == 0) {
            constexpr bool needs_fixup = false; // CUDA block is working on an entire tile.
            flash_attn_ext_f16_process_tile<DKQ, DV, ncols1, ncols2, BERT_TUNED, nwarps, use_logit_softcap, V_is_K_view, needs_fixup, is_fixup>
...
template <int DKQ, int DV, int ncols1, int ncols2, bool BERT_TUNED>
void ggml_cuda_flash_attn_ext_mma_f16_case(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int  nbatch_fa      = ggml_cuda_fattn_mma_get_nbatch_fa_tuned<BERT_TUNED>(DKQ, DV, ncols, cc);
    ...
    if (logit_softcap == 0.0f) {
        ...
        fattn_kernel = flash_attn_ext_f16<DKQ, DV, ncols1, ncols2, BERT_TUNED, use_logit_softcap, V_is_K_view>;
    ...
    launch_fattn<DV, ncols1, ncols2>
        (ctx, dst, fattn_kernel, nwarps, nbytes_shared_total, nbatch_fa, true, true, true, warp_size_host, BERT_TUNED);
}


#define DECL_FATTN_MMA_F16_CASE(DKQ, DV, ncols1, ncols2)                          \
    template void ggml_cuda_flash_attn_ext_mma_f16_case                           \
    <DKQ, DV, ncols1, ncols2, false>(ggml_backend_cuda_context & ctx, ggml_tensor * dst) \

#define DECL_FATTN_MMA_F16_CASE_BERT(DKQ, DV, ncols1, ncols2)                     \
    template void ggml_cuda_flash_attn_ext_mma_f16_case                           \
...
// BGE/BERT 16-head, 64-dimension specialization with the tuned nbatch_fa.
extern DECL_FATTN_MMA_F16_CASE_BERT(64, 64, 64, 1);
