...
// Find a coarse valid K interval for every Q tile in a non-causal encoder
// mask. This is used by the BERT-specific whole-output-tile schedule below:
// unlike dense stream-K, every CUDA block owns the complete reduction for an
// output tile, so trimming both masked edges cannot change fixup ownership.
template <int ncols1>
__launch_bounds__(FATTN_KQ_STRIDE/2, 1)
static __global__ void flash_attn_mask_to_KV_bounds(
        const half * mask_ptr, int * KV_bounds_ptr, const int ne30, const int ne31,
        const int64_t s31, const int64_t s33) {
    const int tid      = threadIdx.x;
    const int sequence = blockIdx.y;
    const int jt       = blockIdx.x;
    const int nrows    = min(ncols1, ne31 - jt*ncols1);

    const half * GGML_CUDA_RESTRICT mask = mask_ptr + sequence*s33 + int64_t(jt)*ncols1*s31;
    int        * GGML_CUDA_RESTRICT bounds = KV_bounds_ptr;
    ...
    const int nchunks = (ne30 + FATTN_KQ_STRIDE - 1) / FATTN_KQ_STRIDE;

    int KV_min = 0;
    for (int chunk = 0; chunk < nchunks; ++chunk) {
        const int k0 = chunk*FATTN_KQ_STRIDE + 2*tid;
        int all_inf = 1;

        // The no-cache BERT mask is block diagonal with contiguous sequence
        // ranges. The first Q row therefore owns the left edge of the union
        // for this tile; scanning the other rows repeats the same search.
        if (k0 + 0 < ne30) {
            all_inf &= int(isinf(__half2float(mask[k0 + 0])));
        }
        if (k0 + 1 < ne30) {
            all_inf &= int(isinf(__half2float(mask[k0 + 1])));
        }
    ...
    for (int chunk = nchunks - 1; chunk >= 0; --chunk) {
        const int k0 = chunk*FATTN_KQ_STRIDE + 2*tid;
        int all_inf = 1;
        const half * last_row = mask + int64_t(nrows - 1)*s31;

        // Symmetrically, the last Q row owns the right edge of the union.
        if (k0 + 0 < ne30) {
            all_inf &= int(isinf(__half2float(last_row[k0 + 0])));
        }
        if (k0 + 1 < ne30) {
            all_inf &= int(isinf(__half2float(last_row[k0 + 1])));
        }
        ...
        if (!all_inf) {
            KV_max = min(ne30, (chunk + 1)*FATTN_KQ_STRIDE);
            break;
        }
    }

    if (tid == 0) {
        const int index = sequence*gridDim.x + jt;
        const int count = gridDim.x*gridDim.y;
        bounds[index]         = KV_max;
        bounds[count + index] = KV_min;
    }
}
...
void launch_fattn(
    ...
    const int warp_size = WARP_SIZE, const bool bert_sparse = false
) {
    constexpr int ncols = ncols1 * ncols2;
    ...
    // Long no-cache BERT batches concatenate many independent documents and
    // produce a block-diagonal mask. Give each output tile to one CUDA block
    // and trim its K loop to the union of non-masked columns. This is the FA2-
    // style alternative to compressing the full dense QxK space into a single
    // persistent stream-K wave.
    const bool use_bert_sparse = bert_sparse && mask && Q->ne[1] >= 1024 && Q->ne[3] == 1;
    ...
    if (use_bert_sparse) {
        ...
        const dim3 blocks_num_KV_bounds(ntiles_x, Q->ne[3], 1);
        const dim3 block_dim_KV_bounds(FATTN_KQ_STRIDE/2, 1, 1);
        const int ne_KV_bounds = blocks_num_KV_bounds.x*blocks_num_KV_bounds.y;

        KV_max.alloc(2*ne_KV_bounds);
        const ggml_cuda_kernel_launch_params launch_params =
            ggml_cuda_kernel_launch_params(blocks_num_KV_bounds, block_dim_KV_bounds, 0, main_stream);
        ggml_cuda_kernel_launch(flash_attn_mask_to_KV_bounds<ncols1>, launch_params,
            (const half *) mask->data, KV_max.ptr, K->ne[1], Q->ne[1], s31, s33);
        CUDA_CHECK(cudaGetLastError());
    } else if (mask && K->ne[1] % FATTN_KQ_STRIDE == 0 && (Q->ne[1] >= 1024 || Q->ne[3] > 1)) {
    ...
    if (stream_k) {
        ...
        // Ada currently prefers stream-K unconditionally.  Keep that default,
        // but allow shape-specific profiling to compare the regular tiled
        // schedule without changing the normal backend behavior.
        const bool disable_stream_k = std::getenv("GGML_CUDA_DISABLE_FA_STREAM_K") != nullptr;
        const bool use_stream_k = !use_bert_sparse && !disable_stream_k &&
