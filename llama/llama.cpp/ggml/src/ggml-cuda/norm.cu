...
static __global__ void norm_f32(
    ...
    for (int col = tid; col < ncols; col += block_size) {
        if constexpr (do_multiply && do_add) {
            ...
            // Preserve the FP32 rounding boundaries of the original NORM -> MUL -> ADD graph.
            // In particular, prevent -use_fast_math from contracting the scale and bias into an FMA.
            const float normalized = __fmul_rn(__fsub_rn(x[col], mean), inv_std);
            const float scaled     = __fmul_rn(normalized, mul[mul_col]);
            dst[col]               = __fadd_rn(scaled, add[add_col]);
...
static void norm_mul_add_f32_cuda(
    ...
    if (ncols < 1024) {
        ...
        const ggml_cuda_kernel_launch_params launch_params = {
            blocks_num, block_dims, block_dims.x > WARP_SIZE ? 32 * sizeof(float2) : 0, stream};
        ggml_cuda_kernel_launch(norm_f32<256, true, true>, launch_params,
        ...
        const dim3 block_dims(1024, 1, 1);
        const ggml_cuda_kernel_launch_params launch_params = {
            blocks_num, block_dims, 32 * sizeof(float2), stream};
...
void ggml_cuda_op_norm_fused_add(ggml_backend_cuda_context & ctx,
    ...
    const ggml_tensor * norm_src = norm_tensor->src[0];
    float eps = 0.0f;
    ...
    if (mul_tensor->src[0] == norm_tensor) {
        ...
        GGML_ABORT("LayerNorm fusion: MUL does not consume NORM");
    }
    ...
    if (add_tensor->src[0] == mul_tensor) {
        ...
        GGML_ABORT("LayerNorm fusion: ADD does not consume MUL");
    }
    ...
    norm_mul_add_f32_cuda(
        (const float *) norm_src->data,
        (const float *) mul_src->data,
        (const float *) add_src->data,
        (float *) add_tensor->data,
        norm_src->ne[0], norm_src->ne[1], norm_src->ne[2], norm_src->ne[3],
        norm_src->nb[1] / ts0, norm_src->nb[2] / ts0, norm_src->nb[3] / ts0,
        mul_src->nb[1] / ts_mul, mul_src->nb[2] / ts_mul, mul_src->nb[3] / ts_mul,
        mul_src->ne[0], mul_src->ne[1], mul_src->ne[2], mul_src->ne[3],
        add_src->nb[1] / ts_add, add_src->nb[2] / ts_add, add_src->nb[3] / ts_add,
        add_src->ne[0], add_src->ne[1], add_src->ne[2], add_src->ne[3],
        eps, ctx.stream());
}
