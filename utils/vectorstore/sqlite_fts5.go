...
func (s *SQLiteFTS5Store) Search(ctx context.Context, req StoreRequest) (gorm.DB, error) {
	...
	matchQuery := sqliteFTS5MatchQuery(queryText)
	if matchQuery == "" {
		return *req.Db.WithContext(ctx).Where("1 = 0"), nil
	}
	...
	ftsQuery := req.Db.Session(&gorm.Session{NewDB: true}).Unscoped().WithContext(ctx).Table(s.Table).
		...
		Where(s.Table+" MATCH ?", matchQuery).
...
func sqliteFTS5MatchQuery(query string) string {
	terms := LexicalQueryTerms(query)
	quoted := make([]string, 0, len(terms))
	for _, term := range terms {
		// The trigram tokenizer cannot satisfy tokens shorter than three runes.
		if len([]rune(term)) < 3 {
			continue
		}
