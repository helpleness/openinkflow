...
func TestDispatcherCachesQueryAndInvalidatesAfterMutation(t *testing.T) {
	...
	if err := registry.Register(orchestrator.Tool{Name: "document.search", Kind: orchestrator.KindQuery, Handler: func(context.Context, json.RawMessage) (any, error) {
		return map[string]any{"call": queryCalls.Add(1)}, nil
	}}); err != nil {
		t.Fatal(err)
	}
	if err := registry.Register(orchestrator.Tool{Name: "fact.create", Kind: orchestrator.KindMutation, Handler: func(context.Context, json.RawMessage) (any, error) {
	...
	first, firstTrace, err := dispatcher.Execute(context.Background(), "alice", "document.search", args)
	if err != nil {
		t.Fatal(err)
	}
	second, secondTrace, err := dispatcher.Execute(context.Background(), "alice", "document.search", args)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, _, err = dispatcher.Execute(context.Background(), "alice", "document.search", args); err != nil {
		t.Fatal(err)
	}
...
func TestDispatcherSingleflightMergesIdenticalQueries(t *testing.T) {
	...
	for range 2 {
		go func() {
			_, trace, err := dispatcher.Execute(context.Background(), "alice", "document.search", json.RawMessage(`{"query":"同一请求"}`))
...
func newTestDispatcher(t *testing.T) (*Dispatcher, *orchestrator.Registry, func()) {
	t.Helper()
