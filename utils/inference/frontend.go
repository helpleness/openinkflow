...
const (
	frontendRerankTimeout          = 70 * time.Second
	frontendRerankMaxDocs          = 24
...
func (p FrontendProvider) Rerank(ctx context.Context, query string, docs []string, topN int) ([]ScoredDocument, error) {
	if len(docs) > frontendRerankMaxDocs {
		return nil, fmt.Errorf("frontend rerank candidate count %d exceeds limit %d", len(docs), frontendRerankMaxDocs)
	}
	if topN > len(docs) {
		topN = len(docs)
	}
