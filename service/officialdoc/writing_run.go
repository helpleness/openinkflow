...
func (service *WritingRunService) runMCPStage(ctx context.Context, run *model.WritingRun) error {
	...
	registry, toolName, err := service.registryForStep(run, round)
	if err != nil {
		return err
	}
	dispatcher, err := executor.NewDispatcher(registry, executor.DispatchOptions{
	...
	result, err := orchestrator.RunWithTools(ctx, []llmutil.Message{
	...
	}, registry, orchestrator.RunOptions{
		UserName: strconv.FormatUint(uint64(run.StartedBy), 10),
		// Dispatcher is the only execution path. The service registers its
		// allow-listed writing tools in the Registry before each controlled run.
		Executor:             dispatcher,
		MaxToolCalls:         1,
		MaxLLMToolCalls:      1,
		...
		OnEvent: func(event string, payload any) {
			switch event {
			case "tool_done":
				if trace, ok := payload.(orchestrator.Trace); ok {
					_ = service.appendTrace(context.Background(), run.ID, round, trace)
				}
			case "tool_error":
				if data, ok := payload.(map[string]any); ok {
					trace := orchestrator.Trace{ToolName: fmt.Sprint(data["tool_name"]), Kind: orchestrator.KindQuery, Status: "error", Error: fmt.Sprint(data["error"]), CreatedAt: time.Now()}
					_ = service.appendTrace(context.Background(), run.ID, round, trace)
				}
			}
		},
...
func (service *WritingRunService) registryForStep(run *model.WritingRun, round int) (*orchestrator.Registry, string, error) {
	registry := orchestrator.NewRegistry()
	register := func(name string, kind orchestrator.Kind, description string, handler orchestrator.Handler) error {
	...
	case writingStepRetrieveEvidence:
		...
		return registry, name, register(name, orchestrator.KindMutation, "从当前任务所属组织的知识库检索并冻结写作证据。无需参数；绝不删除文档或索引。", func(ctx context.Context, _ json.RawMessage) (any, error) {
			return service.retrieveEvidence(ctx, run.ID)
		})
	case writingStepComposeDocument:
		name := "writing.compose_document"
		return registry, name, register(name, orchestrator.KindLLM, "基于已冻结证据生成当前阶段的中文 Markdown；证据不足时可通过只读 knowledge.search 继续检索。无需参数；不会提交或覆盖版本。", func(ctx context.Context, _ json.RawMessage) (any, error) {
			return service.composeDocument(ctx, run.ID, round)
		})
	case writingStepCommitVersion:
		name := "writing.commit_version"
		return registry, name, register(name, orchestrator.KindMutation, "将已生成正文和已冻结证据固化为新的不可变版本。无需参数；不会删除任何版本、任务或文档。", func(ctx context.Context, _ json.RawMessage) (any, error) {
...
func (service *WritingRunService) composeDocument(ctx context.Context, runID uint, round int) (any, error) {
	run, err := service.findRun(ctx, runID)
	if err != nil {
		return nil, err
	}
	...
	remainingSearchCalls, err := service.remainingKnowledgeSearchCalls(ctx, run.ID)
	if err != nil {
		return nil, err
	}
	registry := orchestrator.NewRegistry()
...
func (service *WritingRunService) appendTrace(ctx context.Context, runID uint, round int, trace orchestrator.Trace) error {
