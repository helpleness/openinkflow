...
// WritingTaskService implements the explicit outline → draft workflow. Model
// outputs remain immutable, while the one user-owned manual revision can be
// updated in place as a working copy.
...
// UpdateManualVersion updates an existing manual working copy in place. This
// intentionally does not create another history entry for every autosave;
// generated outline and draft versions are never editable through this path.
func (service *WritingTaskService) UpdateManualVersion(ctx context.Context, tenantID, taskID, versionID, userID uint, input request.DocumentVersionCreate) (response.WritingTaskView, error) {
	task, err := service.findTaskForMember(ctx, tenantID, taskID, userID)
	if err != nil {
		return response.WritingTaskView{}, err
	}
	...
	if err := global.GVA_DB.WithContext(ctx).Where("id = ? AND task_id = ? AND tenant_id = ?", versionID, task.ID, tenantID).First(&version).Error; err != nil {
		return response.WritingTaskView{}, err
	}
	if version.Stage != "manual" {
		return response.WritingTaskView{}, fmt.Errorf("仅人工修订版本可以直接修改；模型生成版本请先另存为人工修订")
	}
	if err := global.GVA_DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&version).Update("content", content).Error; err != nil {
			return err
		}
		return tx.Model(task).Updates(map[string]any{"status": "drafted", "current_version_id": version.ID}).Error
	}); err != nil {
		return response.WritingTaskView{}, err
	}
	return service.Get(ctx, tenantID, taskID, userID)
