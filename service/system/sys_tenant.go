...
func (s *SysTenantService) CreateTenant(ctx context.Context, ownerID uint, name, code string) (*response.SysTenantBootstrap, error) {
	...
	if code == "" {
		// A user may create more than one workspace. Generate a valid unique
		// code when the optional human-facing code is omitted.
		code = fmt.Sprintf("tenant-%d-%d", ownerID, time.Now().UnixNano())
	}
...
func (s *SysTenantService) ListTenants(ctx context.Context, userID uint) ([]model.SysTenant, error) {
	...
	load := func() error {
		return db.WithContext(ctx).
			Table("sys_tenants").
			Joins("JOIN sys_memberships AS membership ON membership.tenant_id = sys_tenants.id AND membership.deleted_at IS NULL").
			Joins("JOIN sys_roles AS role ON role.id = membership.role_id AND role.deleted_at IS NULL").
			Where("sys_tenants.deleted_at IS NULL AND membership.user_id = ? AND membership.status = ?", userID, model.UserStatusActive).
			Order("sys_tenants.name").
			Find(&tenants).Error
	}
	if err := load(); err != nil || len(tenants) > 0 || userID == 0 {
		return tenants, err
	}
...
// UpdateTenantSettings controls self-service workspace applications. A visible
// workspace must have a role from the same tenant for approved applicants.
func (s *SysTenantService) UpdateTenantSettings(ctx context.Context, tenantID, actorID, defaultRoleID uint, visible bool) (*model.SysTenant, error) {
	...
	if err := db.WithContext(ctx).Where("id = ? AND status = ?", tenantID, model.UserStatusActive).First(&tenant).Error; err != nil {
		return nil, errors.New("工作空间不存在或已停用")
	}
	if visible {
		if defaultRoleID == 0 {
			return nil, errors.New("公开工作空间前请先选择默认分配角色")
		}
		var role model.SysRole
		if err := db.WithContext(ctx).Where("id = ? AND tenant_id = ?", defaultRoleID, tenantID).First(&role).Error; err != nil {
			return nil, errors.New("默认角色不存在或不属于当前工作空间")
		}
	}
	if err := db.WithContext(ctx).Model(&tenant).Updates(map[string]any{
		"is_visible":      visible,
		"default_role_id": defaultRoleID,
	}).Error; err != nil {
		return nil, err
	}
	tenant.IsVisible = visible
