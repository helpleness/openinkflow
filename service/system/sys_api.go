...
func apiMetadata(path string) string {
	switch {
	case strings.Contains(path, "/workspace-applications"):
		return "工作空间申请"
	case strings.Contains(path, "/public-workspaces") || strings.Contains(path, "/tenant-settings"):
		return "工作空间管理"
	...
	case strings.Contains(path, "/model-settings") || strings.Contains(path, "/inference/"):
		return "模型配置"
	case strings.Contains(path, "/mcp-servers"):
		return "MCP 配置"
...
func apiDescription(method, path string) string {
	...
	purposes := map[string]string{
		...
		"GET /system/public-workspaces":                                  "已登录账号读取允许公开申请加入的工作空间；不需要现有成员关系。",
		"GET /system/workspace-applications":                             "已登录账号读取自己的工作空间加入申请；不需要现有成员关系。",
		"POST /system/workspace-applications":                            "已登录账号向公开工作空间提交加入申请；工作空间由服务端校验。",
		"GET /system/workspace-applications/reviews":                     "管理员或所有者读取当前工作空间的加入申请。",
		"PUT /system/workspace-applications/:id":                         "管理员或所有者审核工作空间申请；批准时授予当前配置的默认角色。",
		"GET /system/tenant-settings":                                    "读取当前工作空间的公开申请与默认角色设置。",
		"PUT /system/tenant-settings":                                    "设置当前工作空间是否公开申请，以及审核通过时分配的默认角色。",
		...
		"POST /system/tenants":                                           "为当前已登录账号创建工作空间、根组织、所有者角色和所有者成员关系。",
		...
		"GET /system/mcp-servers":                                        "读取当前用户在当前工作空间保存的远程 MCP 服务配置；Bearer Token 只返回是否已保存。",
		"POST /system/mcp-servers":                                       "创建当前用户的远程 Streamable HTTP MCP 服务配置；仅允许 HTTPS 公网端点，不启动本机进程。",
		"PUT /system/mcp-servers/:id":                                    "更新当前用户的远程 MCP 服务配置；提交 bearer_token 会整体替换已保存的 Token。",
		"DELETE /system/mcp-servers/:id":                                 "永久删除当前用户的远程 MCP 服务配置及其保存的 Bearer Token。",
		...
		"POST /officialdoc/writing-tasks/:id/versions":                   "首次保存人工修订，创建一个可持续编辑的人工修订版本。",
		"PUT /officialdoc/writing-tasks/:id/versions/:version_id":        "更新已有的人工修订工作版本，不新增版本记录。",
