...
func TestLocalRegistrationCreatesTenantSessionAndPolicy(t *testing.T) {
	...
	readerRole, err := ServiceGroupApp.SysRoleService.CreateRole(ctx, tenants[0].ID, "只读成员", model.RoleReader, "仅查看知识证据", []string{"workspace", "knowledge_base", "knowledge_search"}, nil)
	if err != nil {
		t.Fatalf("CreateRole(reader) error = %v", err)
	}
	// Registration only creates a global account. A memberless account must not
	// silently acquire a workspace just because a reader role later exists.
	memberTenants, err := ServiceGroupApp.SysTenantService.ListTenants(ctx, member.User.ID)
	if err != nil || len(memberTenants) != 0 {
		t.Fatalf("ListTenants() unassigned registration = %#v, %v", memberTenants, err)
	}
	reader, err := ServiceGroupApp.SysAuthService.RegisterLocal(ctx, "read-only-member", "correct-horse-battery")
	if err != nil {
		t.Fatalf("RegisterLocal(reader) error = %v", err)
	}
	...
	if _, err := ServiceGroupApp.SysMembershipService.AddMembership(ctx, tenants[0].ID, organizations[0].ID, reader.User.ID, readerRole.ID); err != nil {
		t.Fatalf("grant registered reader: %v", err)
	}
	readerAccess, err := ServiceGroupApp.SysRoleService.AccessForUser(ctx, tenants[0].ID, reader.User.ID)
	if err != nil {
		t.Fatalf("AccessForUser(reader) error = %v", err)
	}
	if readerAccess.RoleID != readerRole.ID || readerAccess.RoleCode != model.RoleReader || len(readerAccess.Menus) != 3 || readerAccess.Menus[2] != "knowledge_search" {
		t.Fatalf("reader menu access = %#v, want configured reader menus", readerAccess)
	}
	...
	if err != nil || len(memberships) != 3 || reassigned.RoleCode != model.RoleAdmin {
