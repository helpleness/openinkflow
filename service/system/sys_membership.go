...
// ListGlobalUsers returns members of the current workspace plus accounts that
// have not joined any active workspace. Accounts belonging only to another
// workspace are intentionally excluded to preserve tenant isolation.
func (s *SysUserService) ListGlobalUsers(ctx context.Context, tenantID, actorID uint) ([]response.SysUserDirectoryItem, error) {
	...
	err := db.WithContext(ctx).Table("sys_users AS user_account").
		...
		Joins("LEFT JOIN sys_memberships AS membership ON membership.user_id = user_account.id AND membership.tenant_id = ? AND membership.status = ? AND membership.deleted_at IS NULL", tenantID, model.UserStatusActive).
		...
		Where(`user_account.deleted_at IS NULL AND (
			membership.id IS NOT NULL OR NOT EXISTS (
				SELECT 1 FROM sys_memberships AS any_membership
				WHERE any_membership.user_id = user_account.id
				  AND any_membership.status = ?
				  AND any_membership.deleted_at IS NULL
			)
		)`, model.UserStatusActive).
		Order("user_account.username").Scan(&users).Error
	return users, err
}
...
func (s *SysMembershipService) saveMembership(ctx context.Context, tenantID, organizationID uint, user *model.SysUser, roleID uint) (*model.SysMembership, error) {
	...
	if previousFound && previous.RoleID != 0 && previous.RoleID != roleID {
