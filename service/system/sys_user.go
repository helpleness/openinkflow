...
func (s *SysAuthService) RegisterLocal(ctx context.Context, username, password string) (*response.SysAuthResult, error) {
	...
	// Registration creates only a global account. Workspace access is granted
	// after a workspace administrator approves an application or assigns a
	// membership explicitly; this keeps tenant boundaries meaningful.
