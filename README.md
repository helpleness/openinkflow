...
- [x] `WritingRun` 将一次生成保存为可恢复的 MCP 运行：每一阶段均通过 `Registry → Dispatcher → RunWithTools` 调用受限工具；只注册“检索证据、生成文稿、固化版本”三项工具，不向 MCP 暴露删除能力。
...
- 生产环境请修改 `config.docker.yaml` 中的数据库密码和 JWT 密钥。
...
只需要在 `config.yaml` 中填写部署相关的 OAuth 凭据：
