...
func initializeSystemModule(db *gorm.DB) {
	...
	if err := migrateModelProviderColumns(db); err != nil {
		panic(fmt.Errorf("migrate model provider columns: %w", err))
	}
	if err := db.AutoMigrate(
		&system.SysUser{}, &system.SysTenant{}, &system.SysOrganization{}, &system.SysRole{},
		&system.SysMembership{}, &system.SysMembershipApplication{}, &system.SysTenantApplication{}, &system.SysSession{}, &system.SysAuthChallenge{}, &system.SysAuditLog{}, &system.SysCasbinRule{}, &system.SysApi{}, &system.SysMenu{}, &system.SysModelSetting{}, &system.SysMCPServer{}, &system.SysOAuthUser{},
	); err != nil {
...
// migrateModelProviderColumns is an explicit, additive migration for the
// provider_type fields. Existing URL-only records intentionally remain blank:
// the provider factory treats that legacy state as openai-compatible without
// guessing from the stored URL, and the next save writes an explicit choice.
func migrateModelProviderColumns(db *gorm.DB) error {
	if db == nil {
		return gorm.ErrInvalidDB
	}
	if !db.Migrator().HasTable(&system.SysModelSetting{}) {
		return nil
	}
	for _, column := range []string{"provider_type", "ocr_provider_type"} {
		if db.Migrator().HasColumn(&system.SysModelSetting{}, column) {
			continue
		}
		if err := db.Migrator().AddColumn(&system.SysModelSetting{}, column); err != nil {
			return fmt.Errorf("add sys_model_settings.%s: %w", column, err)
		}
	}
	return nil
}
