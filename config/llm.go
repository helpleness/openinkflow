...
type LLM struct {
	// ProviderType is explicit by design. Empty legacy values are treated as
	// "openai-compatible" by the provider factory, never inferred from URL.
	ProviderType      string  `mapstructure:"provider-type" json:"provider-type" yaml:"provider-type"`
