...
// GetTenantSettings returns the active workspace's self-service join policy.
func (api *SysTenantApi) GetTenantSettings(c *gin.Context) {
	items, err := systemService.ServiceGroupApp.SysTenantService.ListTenants(c.Request.Context(), ginctx.CurrentUserID(c))
	if err != nil {
	...
	for _, item := range items {
		if item.ID == ginctx.CurrentTenantID(c) {
			commonResponse.Respond(item, nil, commonResponse.ErrForbidden, c)
			return
		}
	}
	commonResponse.Respond(nil, commonResponse.ErrForbidden, commonResponse.ErrForbidden, c)
}

// UpdateTenantSettings configures public workspace applications and the role
// received by an approved applicant.
func (api *SysTenantApi) UpdateTenantSettings(c *gin.Context) {
	var req request.SysTenantSettingsUpdate
	if err := c.ShouldBindJSON(&req); err != nil {
		commonResponse.BadRequest("请求参数无效", c)
		return
	}
	item, err := systemService.ServiceGroupApp.SysTenantService.UpdateTenantSettings(c.Request.Context(), ginctx.CurrentTenantID(c), ginctx.CurrentUserID(c), req.DefaultRoleID, req.IsVisible)
