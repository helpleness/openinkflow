...
func (api *WritingTaskApi) UpdateManualVersion(c *gin.Context) {
	...
	if err != nil || versionID == 0 {
		commonResponse.BadRequest("无效的人工修订版本 ID", c)
		return
	}
	var req request.DocumentVersionCreate
	if err := c.ShouldBindJSON(&req); err != nil {
	...
	item, err := service.ServiceGroupApp.WritingTaskService.UpdateManualVersion(c.Request.Context(), ginctx.CurrentTenantID(c), uint(taskID), uint(versionID), ginctx.CurrentUserID(c), req)
	commonResponse.Respond(item, err, commonResponse.ErrForbidden, c)
}

// ExportVersion streams Markdown, DOCX or PDF for a stored version.
